...
class SEHF(nn.Module):
    ...
    def forward(self, event_first, rgb_first, event_input):
        clipper = self.REClipper(event_first, rgb_first) * self.manifier # 2*24*280*448
        event_input = torch.cat([event_input, clipper], dim=1)  # 4*24*448*280

        x = self.generator(event_input)  # 4*24*448*280
